...
/* StateRAM-13: learned object-transition predictor + cheap approximate LRU. */
static uint32_t transition_count[MAX_OBJECTS][MAX_OBJECTS];
static uint64_t predictor_total=0, predictor_hits=0;
static int last_object=-1, last_prediction=-1;
static int evict_cursor_o=0;
static uint64_t evict_cursor_s=0;
static double prefetch_cost_ewma_ms=1.5;
...
static size_t seg_offset(int oi,uint64_t seg) {
    (void)oi;
    return (size_t)seg*cfg.segment_kb*1024UL;
}

static size_t seg_len(int oi,uint64_t seg) {
    size_t off=seg_offset(oi,seg);
    size_t segb=cfg.segment_kb*1024UL;
...
static int make_room(size_t need,int protect_o,uint64_t protect_seg,double *evict_ms) {
    ...
    while(tracked_raw_bytes+need>target) {
        ...
        /*
         * StateRAM-13: sample up to 512 segment slots from a rotating cursor
         * instead of scanning the whole logical universe for every fine-grained
         * eviction.  With a ~3 GiB window this still samples many resident
         * candidates, while keeping foreground eviction selection bounded.
         */
        size_t total_slots=0;
        for(int oi=0;oi<cfg.objects;oi++) total_slots += objs[oi].segments;
        size_t sample = total_slots < 512 ? total_slots : 512;

        int oi=evict_cursor_o;
        uint64_t ss=evict_cursor_s;
        for(size_t k=0;k<sample;k++) {
            while(oi>=cfg.objects || ss>=objs[oi].segments) {
                if(oi>=cfg.objects) oi=0;
                if(ss>=objs[oi].segments) { oi++; ss=0; }
            }
        ...
        /* Rare fallback when the sample happened to miss all residents. */
        if(best_o<0) {
            for(int x=0;x<cfg.objects && best_o<0;x++) {
                for(uint64_t y=0;y<objs[x].segments;y++) {
                    if(!objs[x].resident[y]) continue;
                    if(x==protect_o && (y==protect_seg ||
                        y+1==protect_seg || protect_seg+1==y)) continue;
                    best_o=x; best_s=y; break;
                }
            }
        }

        if(best_o<0) { errno=ENOMEM; return -1; }
        if(evict_segment(best_o,best_s,evict_ms)!=0) return -1;
    }
    return 0;
}
...
static int init_objects(void) {
    ...
    if((cfg.segment_kb*1024UL) < page_size ||
       (cfg.segment_kb*1024UL) % page_size != 0) {
        errno=EINVAL;
        fprintf(stderr,"segment size must be page-aligned\n");
        return -1;
    }
...
static void observe_transition(int from,int to) {
    if(from<0 || to<0 || from>=cfg.objects || to>=cfg.objects || from==to)
        return;
    uint32_t *v=&transition_count[from][to];
    if(*v<UINT32_MAX) (*v)++;

    /* Very cheap aging so stale routes eventually lose influence. */
    if(*v>1000000U) {
        for(int j=0;j<cfg.objects;j++)
            transition_count[from][j]=(transition_count[from][j]+1U)/2U;
    }
}
...
static int predicted_next_object(int current) {
    ...
    for(int j=0;j<cfg.objects;j++) {
        ...
        if(transition_count[current][j]>score) {
            score=transition_count[current][j];
            best=j;
        }
    }
...
static void usage(const char *p) {
    fprintf(stderr,
      ...
      "  --objects N\n"
      "  --segment-kb N\n"
      "  --segment-mb N  (compatibility alias)\n"
      "  --raw-target-mb N\n"
      "  --shadow-depth N\n"
      "  --steps N\n"
...
static int parse_args(int argc,char **argv) {
    for(int i=1;i<argc;i++) {
        ...
        else if(strcmp(argv[i],"--segment-kb")==0 && i+1<argc) cfg.segment_kb=strtoull(argv[++i],NULL,10);
        else if(strcmp(argv[i],"--segment-mb")==0 && i+1<argc) cfg.segment_kb=1024UL*strtoull(argv[++i],NULL,10);
    ...
    if(cfg.objects<2 || cfg.objects>MAX_OBJECTS ||
       cfg.logical_gb<=0 || cfg.segment_kb<4 ||
...
int main(int argc,char **argv) {
    ...
    for(int step=0;step<cfg.steps;step++) {
        ...
        if(last_prediction>=0 && switched) {
            predictor_total++;
            if(current==last_prediction) predictor_hits++;
        }
        if(last_object>=0 && switched && current!=last_object)
            observe_transition(last_object,current);
        last_object=current;
        ...
        if(strcmp(cfg.mode,"stateram")==0) {
            /*
             * StateRAM-13 execution shadowing.  Never begin a background
             * reconstruction when the remaining user-think-time is smaller
             * than our learned reconstruction cost plus a small safety margin.
             */
            for(int d=1;d<=cfg.shadow_depth;d++) {
                uint64_t ns=cur_seg+(uint64_t)d;
                if(ns>=objs[current].segments) break;
                if(objs[current].resident[ns]) continue;

                double remain=next_deadline-now_ms();
                if(remain < prefetch_cost_ewma_ms + 1.5) break;

                double e=0;
                double p0=now_ms();
                if(materialize_segment(current,ns,&e)!=0) break;
                double cost=now_ms()-p0;
                prefetch_cost_ewma_ms=0.85*prefetch_cost_ewma_ms+0.15*cost;
                total_evict+=e;
            }

            /* Learned next-object entry capsule. */
            int po=predicted_next_object(current);
            last_prediction=po;
            if(!objs[po].resident[0]) {
                double remain=next_deadline-now_ms();
                if(remain >= prefetch_cost_ewma_ms + 1.5) {
                    double e=0;
                    double p0=now_ms();
                    if(materialize_segment(po,0,&e)==0) {
                        double cost=now_ms()-p0;
                        prefetch_cost_ewma_ms=
                            0.85*prefetch_cost_ewma_ms+0.15*cost;
                        total_evict+=e;
                    }
                }
            }
    ...
    fprintf(stderr,
      "Done.\n"
      " mode=%s logical=%.2f GiB objects=%d segment=%zu KiB\n"
      " tracked raw target=%zu MiB current tracked=%.1f MiB RSS=%.1f MiB\n"
      " background work=%.1f ms eviction work=%.1f ms\n"
      " predictor hits=%" PRIu64 "/%" PRIu64 " (%.1f%%) prefetch_ewma=%.3f ms\n"
      " CSV=%s\n",
      cfg.mode,cfg.logical_gb,cfg.objects,cfg.segment_kb,
      cfg.raw_target_mb,tracked_raw_bytes/(1024.0*1024.0),
      rss_bytes()/(1024.0*1024.0),
      total_bg,total_evict,predictor_hits,predictor_total,
      predictor_total ? 100.0*(double)predictor_hits/(double)predictor_total : 0.0,
      prefetch_cost_ewma_ms,cfg.csv_path);
